...
setInterval(() => {
    // 10% chance every 2 seconds to trigger a severe page tear
    if (Math.random() > 0.9) {
        document.body.style.transform = `skewX(${Math.random() * 5 - 2.5}deg) scale(1.02)`;
        document.body.style.filter = `hue-rotate(${Math.random() * 90}deg) contrast(120%)`;

        // Fix it a fraction of a second later
        setTimeout(() => {
